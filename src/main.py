...
def change_contact(address_book, contact_name):
    ...
    while True:
        field_to_change = input("Enter the field you want to change (name, phone, email, address, birthday) or 'cancel' to exit: ").lower()
        ...
        if field_to_change == 'name' or field_to_change in 'name':
            ...
            if contact.addresses:
                old_address = input("Enter the old address (format: city street house): ")
            else:
                old_address = None
                    ...
                    if len(new_address_parts) == 3:
                        ...
                        if old_address:
                            ...
                            if len(old_address_parts) == 3:
                                ...
                                print(f"Address changed from {old_address} to {new_address}")
                            else:
...
def change_note(notes, note_text):
    ...
    while True:
        field_to_change = input("Enter the field you want to change (text, tags): ").lower()
...
def delete_note(notes, keyword):
    ...
    if len(found_notes) > 1:
        print("Found multiple notes:")
        for i, note in enumerate(found_notes):
            print(f"{i + 1}. Text: {note[1].text}")
            if note[1].tags:
                print(f"   Tags: {', '.join(note[1].tags)}")
            else:
                print("   Tags: N/A")
        while True:
    ...
    confirm_delete = input("Do you want to delete this note? (yes/no): ").lower()
    if confirm_delete == 'yes' or confirm_delete == 'y':
        del notes.notes[note_index]
        print(f"Note with the keyword '{keyword}' deleted successfully.")
    else:
